<template>
 <div> 
	  ...
	  <div class="z-padding-v-5px z-bg-ff9 z-bold z-margin-top-10px">当让了。也可以在vuex中的 getters中改动后，再在此文件中调用改动的数据，注意，此时有两种方法拿到vuex中的getters的值，第一种就是在computed中返回，第二种就是 ...mapGetters 集中引入，自行查看即可</div>	  
	  <div>
	    <div v-for="(item,inde) in gettersProduc" :key="inde">{{item.name}}价格参数 {{item.price}}</div>
	  </div>	  
	  <div class="z-padding-v-5px z-bg-80d z-bold z-margin-top-10px" @click="reducePrice()">下面演示在本地的methods中定义一个函数，用于修改 vuex中的state中的数据，但是不推荐在本地zhe样子操作，<span class="z-color-f">可点击此div</span></div> 
	  <div class="z-padding-v-5px z-bg-80d z-bold z-margin-top-10px" @click="changePrice()">下面演示在vuex中自定义的改变其state中的数据的方法；在vuex文件中的 mutations 方法，类似于事件触发，<span class="z-color-f">可点击此div</span></div> 
...
<script>
	...
	export default {
		...
		computed:{		
		  ...
		  // ...mapGetters([  //可以同时调用多个此方法，而不是一个一个的写，像上面 需要引入该方法。。
    //      "setProduc"
	  	// ]),		  	
	  	gettersProduc(){ 
        ...
        this.$store.commit('reduce'); // reduce 就是mutation中的事件名了
	  	},	
	    actionsChange(){  
